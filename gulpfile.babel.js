...
gulp.task('copyJsLibs', () => {
  return gulp.src([`${paths.src.javascripts}libs/*`])
    .pipe(gulp.dest(`${paths.dist.javascripts}libs`))
    .on('end', () => {
      if(!isProduction && bs.active) reload()
    })
})
...
gulp.task('default', () => {
  if (!isProduction) {
    runSequence('clean', 'copyFile', 'copyJsLibs', ['styles', 'scripts', 'images:min', 'build'], 'serve', 'watch')
  } else {
    runSequence('clean', 'copyFile', 'copyJsLibs', ['styles', 'scripts', 'images:min', 'build'])
  }
})
